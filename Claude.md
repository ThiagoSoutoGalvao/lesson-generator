...
### Not yet built
- **Part B — speaking practice** (Speak About the Photo, Read Then Speak, Interactive Speaking) — not started; per the roadmap doc these need only a shared read-time/speak-time timer component and no recording/storage at all. All 4 Part A templates are now complete, which was the trigger point noted in the content-strategy decision above to revisit content sourcing (manual batch-authoring vs. an authoring helper vs. live Claude API generation) — worth raising next time content changes are discussed, not decided unilaterally here.
