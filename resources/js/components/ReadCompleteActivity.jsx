...
// DET-style Read and Complete: a passage where each gapped word shows its first
// letters, the rest as underscores. Teacher clicks a gap to reveal the whole
// word. Same reveal model as OpenClozeActivity.
//
// The gap span carries its own horizontal margin so it reads as a separate word
...
// blanks use mx-0.5).
export default function ReadCompleteActivity({ activity, onClose, onComplete, hideSave }) {
    ...
    const processedParts = useMemo(() => {
        ...
        return (activity.parts ?? []).map(part =>
            part.answer !== undefined ? { ...part, blankIndex: counter++ } : part
        );
    }, [activity]);

    const totalBlanks = processedParts.filter(p => p.answer !== undefined).length;
                ...
                <div className="max-w-4xl w-full rounded-2xl bg-black/30 backdrop-blur-sm border border-white/15 px-8 py-6">
                    <p className={`${textColor} ${FONT_SIZES[fontSizeIdx]} leading-loose`}>
                        {processedParts.map((part, i) => {
                            ...
                            if (isRev) {
                                return <mark key={i} className="mx-1 bg-yellow-400/25 text-yellow-300 font-bold rounded px-1 not-italic">{part.answer}</mark>;
                            }
                            const missing = Math.max(1, part.answer.length - part.given.length);
                            return (
                                <span key={i} onClick={() => revealBlank(part.blankIndex)}
                                    className="inline-block mx-1 font-bold text-white/85 hover:text-white transition-colors cursor-pointer"
                                    title="Click to reveal">
                                    {part.given}<span className="text-white/40 tracking-widest">{'_'.repeat(missing)}</span>
                                </span>
                            );
                        })}
                    </p>
