...
import silentLetters from '@/data/pronunciation/silentLetters.json';
...
function buildSession(mode) {
    const pool = mode === 'mixed'
        ? silentLetters.flatMap(g => g.words)
        : silentLetters.find(g => g.category === mode).words;

    const items = shuffle(pool)
        .slice(0, SESSION_SIZE)
        .map((w, i) => {
            const choices = shuffle(w.candidates).map(letter => ({
                key: letter,
                ipa: letter,
            }));
            return {
...
export default function SilentLettersDrill() {
    ...
    return (
        ...
        >
            ...
            <div className="relative z-10 flex items-center justify-between px-8 py-4 border-b border-white/10">
                <div>
                    <h2 className="text-xl font-bold text-white">Silent Letters Drill</h2>
                    <p className="text-white/50 text-xs">
                        {phase === 'select' && 'Pick a category — hear a word, choose which letter is silent'}
