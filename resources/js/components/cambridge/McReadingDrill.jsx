...
export default function McReadingDrill() {
    ...
    function handleAnswer(optIndex) {
        ...
        setAnswers(prev => {
            const next = [...prev];
            next[qIndex] = { given: optIndex, revealed: true, correct: optIndex === question.answerIndex };
            return next;
        });
    }
                    ...
                    <div className="flex flex-col gap-3 max-w-md w-full mx-auto">
                        ...
                        {mcReadingSets.map(s => (
                            <button key={s.id} onClick={() => startSet(s)}
                                ...
                                <p className="text-white/40 text-xs font-normal mt-1">{s.questions.length} questions</p>
                            </button>
                        ))}
                    </div>
                </div>
                        ...
                        <div className="flex-1 overflow-y-auto flex flex-col items-center justify-center gap-6 px-8 py-8">
                            ...
                            {qState.revealed && (
                                <p className={qState.correct ? 'text-green-300 font-semibold' : 'text-red-300 font-semibold'}>
                                    {qState.correct ? 'Correct!' : 'Not quite — check the highlighted answer.'}
                                </p>
                            )}
                        </div>
