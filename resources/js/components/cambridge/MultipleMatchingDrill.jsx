            ...
            {phase === 'select' && (
                <div className="flex-1 overflow-y-auto px-8 py-8">
                    <div className="flex flex-col gap-3 max-w-md w-full mx-auto">
                        <p className="text-white/60 text-sm text-center mb-2">Read the short texts, then match each question to the text it belongs to.</p>
                        {multipleMatchingSets.map(s => (
                ...
                <div key={`${sessionKey}-${qIndex}`} className="flex-1 flex flex-col md:flex-row min-h-0 overflow-hidden">
                    ...
                    <div className="flex-1 flex flex-col min-h-0">
                        <div className="flex-1 overflow-y-auto flex flex-col items-center justify-center gap-6 px-8 py-8">
                            <p className={`${QUESTION_SIZES[fontSizeIdx]} font-semibold text-center max-w-xl ${textColor}`}>{set.instruction} {question.question}</p>
