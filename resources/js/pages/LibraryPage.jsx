...
const filterBtnCls = (active) =>
    `px-4 py-1.5 rounded-lg text-sm font-medium transition-colors cursor-pointer border ${
        active ? 'bg-[#e0521f] border-[#e0521f] text-white shadow-lg shadow-[#fc6840]/30' : 'lg-chip lg-chip-hover text-white/70 hover:text-white'
    }`;

const briefFieldCls = 'bg-white/8 border border-white/12 text-white placeholder:text-white/35 rounded-lg px-3 py-2 text-xs w-full resize-y focus:outline-none focus:ring-2 focus:ring-[#fc6840]';
    ...
    return (
        <div className="flex flex-col gap-3 bg-white/5 rounded-xl p-3 mt-1">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
                {BRIEF_FIELDS.map(([key, label, placeholder]) => (
                    <div key={key} className="flex flex-col gap-1">
                        <label className="text-white/55 text-[11px] font-medium">{label}</label>
                        <textarea
                            value={fields[key]}
            ...
            <div className="flex items-center gap-2 flex-wrap">
                <label className="text-white/55 text-[11px] font-medium">Updated by</label>
                <select
                    value={updatedBy}
                    onChange={e => setUpdatedBy(e.target.value)}
                    className="bg-white/8 border border-white/12 text-white text-xs rounded-lg px-2 py-1.5 cursor-pointer"
                >
                ...
                <button
                    onClick={handleSave}
                    disabled={saving}
                    className="bg-[#e0521f] hover:bg-[#c9461a] disabled:bg-[#e0521f]/30 text-white text-xs font-semibold px-4 py-1.5 rounded-lg transition-colors cursor-pointer"
                >
    ...
    return (
        <div className="lg-surface border rounded-2xl p-4 overflow-x-auto">
            <div className="flex items-center justify-between mb-3 gap-3">
                <h3 className="font-display text-white font-bold text-sm">{meta.label} coverage</h3>
            ...
            <table className="w-full text-xs border-separate border-spacing-1 min-w-[480px]">
                <thead>
                    <tr>
                        <th className="text-left text-white/55 font-medium pr-2">Lesson</th>
                        {LESSON_SLOTS.map(s => (
                                    ...
                                    <td>
                                        <button
                                            ...
                                            className={`text-left font-semibold px-2 py-1 rounded-lg cursor-pointer transition-colors whitespace-nowrap ${
                                                active ? 'bg-[#e0521f] text-white' : 'text-white/80 hover:bg-white/8'
                                            }`}
                                        >
                                            ...
                                            <td key={s.key} className="text-center">
                                                <span
                                                    ...
                                                    className={`inline-flex items-center justify-center w-7 h-7 rounded-md text-sm ${
                                                        filled ? `${meta.accent} text-white` : 'bg-white/5 text-white/30 border border-white/10'
                                                    }`}
                                                >
                                                    {filled ? '✓' : '·'}
                                                </span>
                                    ...
                                    <td className="text-center">
                                        <button
                                            ...
                                            className={`relative inline-flex items-center justify-center w-7 h-7 rounded-md text-sm cursor-pointer transition-colors ${
                                                expanded ? 'bg-[#e0521f] text-white' : 'bg-white/5 text-white/70 hover:bg-white/10 border border-white/10'
                                            }`}
                                        >
                                            📝
        ...
        <div className="flex flex-col gap-6">
            ...
            {hasTrilhaActivities && (
                <div className="flex flex-wrap gap-2 border-t border-white/10 pt-4">
                    <span className="text-white/40 text-xs self-center mr-1">Trilha:</span>
            ...
            {folders.length > 0 && (
                <div className="flex flex-wrap gap-2 border-t border-white/10 pt-4">
                    <span className="text-white/40 text-xs self-center mr-1">Folder:</span>
            ...
            {loading && (
                <div className="flex justify-center py-12">
                    <Spinner message="Loading activities…" color="text-white/70" textColor="text-white/55" />
                </div>
            )}
            ...
            {!loading && !error && filtered.length === 0 && (
                <div className="lg-shell-text text-center py-20 text-white/75">
                    ...
                    >
                        ...
                        {(a.trilha || a.book || a.lesson) && (
                            <div className="flex gap-2 flex-wrap">
                                {a.trilha && (
                                    <span className={`text-xs font-semibold text-white px-2.5 py-1 rounded-full ${TRILHAS[a.trilha]?.accent ?? 'bg-white/8'}`}>
                                        {TRILHAS[a.trilha]?.label ?? a.trilha}{a.trilha_lesson ? ` · L${String(a.trilha_lesson).padStart(2, '0')}` : ''}
                                    </span>
                                )}
                                {a.book && (
                                    <span className="text-xs bg-white/8 text-white/90 px-2.5 py-1 rounded-full border border-white/10">
                                        {a.book}
                                    </span>
                                )}
                                {a.lesson && (
                        ...
                        <div className="flex gap-2 pt-1">
                            ...
                            <button
                                ...
                                className="bg-white/8 hover:bg-red-500 text-white/60 hover:text-white text-sm px-3 py-2.5 rounded-xl transition-colors cursor-pointer border border-white/12"
                            >
                                Delete
                            </button>
                        </div>
