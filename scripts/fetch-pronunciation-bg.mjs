// One-off background image search for the Pronunciation drill pages — same
// candidates/finalize pattern as fetch-det-photos.mjs. Just need one image.
...
const REVIEW_DIR = path.join(ROOT, 'storage/tmp/pronunciation-bg-review');
...
async function runFinalize() {
    ...
    if (!KEEP_KEY) throw new Error('Pass the key to keep: node scripts/fetch-pronunciation-bg.mjs finalize q0-1');
    ...
    const destPath = path.join(FINAL_DIR, 'pronunciation.jpg');
...
else console.log('Usage: node scripts/fetch-pronunciation-bg.mjs [candidates|finalize <key>]');
