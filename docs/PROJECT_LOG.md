# Lesson Generator — Project Log

Full historical record of completed phases. This file is the detailed changelog;
`Claude.md` at the repo root holds the lean, current-state brief and points here.

Moved out of `Claude.md` on 2026-09-09 to keep the always-loaded project brief
under its size limit. Nothing was dropped — every phase note is reproduced verbatim
below. Newest phases are at the bottom (same order as the original document).

Companion spec docs (unchanged, still at repo root): `PronunciationFeature.md`,
`AuroraStudentApp.md`, `CambridgePracticeMode.md`, `CambridgeResearch.md`,
`AuroraBranding.md`, `# DET Practice Mode — Feature Roadmap.md`,
`ProficiencyTestsComparison.md`, `docs/phase11-marketing.md`.
...
Still in progress — lives in `Claude.md` §5, not reproduced here.
...
- **Two follow-up bugs caught by the user, both fixed**:
  - The select screen's `flex items-center justify-center` container (no scroll) was fine at 6 groups but broke the moment the count grew to 10 (5 rows) — the same `items-center` + no-`overflow-y-auto` trap documented in DET's content batch 2 notes: excess content gets centered and pushed equally above/below the viewport, and the bottom half becomes unreachable. Fixed the same way as DET's three select screens: dropped `items-center`/`justify-center` for a top-aligned `overflow-y-auto` container with the button grid centered via `mx-auto` instead.
  - "Still just black" — `.lg-surface-soft`'s alpha (.55) wasn't low enough given `pronunciation.jpg` is mostly solid black with the blue wave occupying a minority of the frame; cards sitting over a black region of the photo necessarily look dark no matter the alpha, since there's no color there to let through. Dropped alpha further to .38 and raised `brightness()` from 1.15 to 1.45 so dark backdrop regions lift a bit rather than reading as flat black. Re-verified via screenshot on both the select screen and the drilling screen (`DrillLoop`'s `softCards` cards use the same token) — clearly more transparent now, text still fully legible.

### Phoneme Drill UX follow-ups ✅ COMPLETED (2 of 3 — third is a scoped content decision, see below)

- **"Choose Another Pair" exit link**: `DrillLoop.jsx` gained an optional `onExit`/`exitLabel` prop pair (both default to not rendering anything) — a text-link button below the choice grid. Only `MinimalPairsDrill.jsx` passes it (`onExit={() => setPhase('select')}`); `EdEndingsDrill.jsx` doesn't, so its drilling screen is unchanged. The existing ✕ button in the header already did this same navigation, but the user wanted it discoverable right at the point of interaction, not just in the corner.
- **Session size 10 → 20**: `SESSION_SIZE` in `MinimalPairsDrill.jsx` bumped to 20. Safe across all 10 groups since every group now has at least 20 words (`.slice(0, 20)` never exceeds what's available — the 4 newest groups have exactly 20, the original 6 have 24). `EdEndingsDrill` has its own separate sizing, untouched.
- **Third ask — showing the actual played word as a choice, paired with a contrasting word** ✅ COMPLETED (light version, chosen over a full data re-pairing pass): investigated before building anything, since it changes the data model. `minimalPairs.json`'s two per-sound word lists were deliberately built as flat categorized lists, not 1:1 pairs (see Phase M1's original design note). Checked several groups' index alignment directly: `/ɪ/ vs /iː/` and `/æ/ vs /ʌ/` happen to be clean 1:1 minimal pairs throughout, but `/θ/ vs /ð/`, `/v/ vs /b/`, `/ʃ/ vs /tʃ/`, `/ɒ/ vs /ʌ/` are a mix — some words align as true minimal pairs by chance (van↔ban, vote↔boat), most don't (save↔cab, marvel↔marble). Presented two options — a light version now vs. a full re-pairing content pass across all 10 groups (worth parallel subagents given the linguistic judgment needed) — user picked the light version.
  - `MinimalPairsDrill.jsx`'s `buildSession()` no longer returns one fixed `choices` array for the whole session. Each item now gets its own `item.choices`: the actual played word as one card, plus a randomly picked word from the *other* sound in the same group as the second card (a real, valid contrasting word — just not guaranteed to be that specific word's true minimal-pair partner), with left/right position randomized each time. `correctKey` changed from the sound symbol to the specific word, since both cards are now specific words.
...
### Word Stress Drill — 5th pronunciation template ✅ COMPLETED (Phases W1–W3)

- Full future-topics catalog (tiered by build cost — reuse-the-pattern-directly / needs-more-design / deferred-pricier) and this drill's spec live in `PronunciationFeature.md` §9–10; picked as the first addition since it's the highest-value gap for Brazilian Portuguese speakers (Portuguese stress placement is far more regular than English's) and reuses the existing `DrillLoop` engine with only a small opt-in prop addition, no new interaction pattern.
- `resources/js/data/pronunciation/wordStress.json` — 3 categories: 2-syllable words (8), 3-syllable words (8), noun/verb stress pairs (10 — 5 pairs: record, present, object, contract, produce). Each entry has `syllables` (array) and `stressIndex`; noun/verb pairs share the same spelling but point at distinct audio files (`record-noun.mp3`/`record-verb.mp3`) since the two forms are pronounced differently.
- **Audio pipeline extended for the noun/verb pairs specifically**: `scripts/generate-pronunciation-audio.mjs`'s `buildWordList()` now also reads `wordStress.json` (skipping the `"word (noun)"`-style pair entries, which aren't plain words). A new `buildStressPairList()`/`generateStressPairs()` path handles the 10 pair words separately — each gets a short context sentence (`STRESS_PAIR_CONTEXT`, e.g. "I broke the world record." vs. "Please record this conversation.") plus an explicit "stress falls on the Nth syllable" instruction derived from that entry's own `stressIndex`, which is what actually locks in the correct reading reliably (TTS defaults to one stress pattern per spelling otherwise).
- **Mistake made and caught during audio generation**: fixing one silent file (`contract-noun.mp3`) was done with `--force`, which regenerates *every* word regardless of whether it already existed — not just the missing one. This silently rewrote all 387 previously-committed word audio files on disk with fresh TTS output (mostly fine-sounding, but pure unnecessary drift — different renders of already-shipped, already-verified audio) and randomly introduced 9 *new* silent files among them (asked, counted, dressed, planned, rained, row, sleep, ten, turned — previously-good audio used by the Phoneme Drill and -ed Endings). Caught in two passes: first the 9 silent files via a folder-wide size scan, fixed by deleting just those and regenerating without `--force`; then a second look at `git status` surfaced the full blast radius — 387 files showing as modified, not 9. Fixed by `git checkout -- public/audio/pronunciation/words/`, which restored every previously-tracked file to its committed version and left only the genuinely new content (this phase's 26 word-stress files) untouched. Worth remembering twice over: `--force` on this script is whole-corpus, not scoped to what you're currently working on, and `git status` — not just a targeted byte-size scan — is the real check for "did this touch anything it shouldn't have" after any bulk regeneration.
- **`DrillLoop.jsx` gained one small backward-compatible prop**: `plainBigText` (default `false`) — when true, the big choice label renders as plain text instead of being wrapped in IPA slashes (`/…/`), since a syllable-stress rendering like `un-DER-stand` isn't a phonemic symbol. Also made the small caption line (`choice.example`) render only when non-empty, since Word Stress's choices don't need one (the big text already shows the full word). Neither Phoneme Drill nor -ed Endings pass `plainBigText`, so their existing slash-wrapped rendering is unchanged — regression-checked via screenshot.
- `resources/js/components/WordStressDrill.jsx` — same fullscreen-overlay/select→drilling→results pattern as `EdEndingsDrill.jsx` (same `pronunciation.jpg` background, `.lg-surface-soft` cards, `softCards` on `DrillLoop`). Each choice card is the word re-rendered once per syllable with a different syllable capitalized (`markStress()`); choice count naturally follows syllable count (2 or 3), which `DrillLoop`'s existing adaptive grid already supports unchanged. Select screen offers Mixed (all 26 words pooled) plus the 3 individual categories, mirroring `EdEndingsDrill`'s Mixed/single-ending pattern.
- Wired in the same way as the other 3 drills: `PronunciationDrillPage.jsx` gained a `type === 'word-stress'` branch, and `UploadPage.jsx`'s `PronunciationLauncher` grid grew from 3 buttons (`sm:grid-cols-3`) to 4 in a 2×2 layout (`sm:grid-cols-2`) — same grid-growth pattern DET's launcher went through when Vocabulary Practice was added as its 8th template.
- Verified via Playwright: ran full sessions for all 3 individual categories plus Mixed, always clicking the first choice — non-hardcoded scores (3/8, 5/10, 8/20) confirm real per-item correctness checking rather than a fixed result; screenshot-reviewed the 3-syllable drilling screen (3 choices, e.g. `UN-der-stand`/`un-DER-stand`/`un-der-STAND`) and the noun/verb pairs screen (2 choices, e.g. `OB-ject`/`ob-JECT`) for correct syllable-marking and to confirm no stray IPA slashes leaked through; confirmed both `object-noun.mp3` and `object-verb.mp3` fetch successfully with distinct file sizes (genuinely different audio, not a duplicate); launcher screenshot confirms the new 2×2 grid. Zero console/page errors.
- **Phase W4 (final verification pass + doc update) folded into this same session** rather than being a separate follow-up — see `PronunciationFeature.md` §10 for the original phase breakdown.

### Word Stress content batch — +10 words per category ✅ COMPLETED

- 2-syllable words: 8 → 18 (added window, children, basket, happen, moment, suggest, control, begin, arrive, complete — 5 initial-stress + 5 final-stress, continuing the deliberate stress-pattern mix from the first batch so the drill isn't guessable from a pattern).
- 3-syllable words: 8 → 18 (added cabinet, vitamin, animal, computer, potato, banana, together, afternoon, kangaroo, seventeen — spanning all three stress positions, including "seventeen" as a classic teen-number third-syllable-stress example).
- Noun/verb stress pairs: 5 → 10 pairs (added convert, export, import, permit, protest — all classic ESL noun/verb stress-shift pairs, same caliber as the original 5). `STRESS_PAIR_CONTEXT` in `scripts/generate-pronunciation-audio.mjs` extended with a context sentence per new pair/part-of-speech, same pattern as the original 5.
- Generated audio via the existing `words` mode **without** `--force` this time (lesson from the earlier incident) — 19 of 20 new plain words created (one, "banana", already existed as a file from unrelated prior content and was correctly skipped rather than regenerated) plus all 10 new stress-pair files; confirmed via `git status` that only new files appeared, nothing existing was touched. Checked the whole `words/` folder (441 files) for the known silent-file bug — none found.
- Verified via Playwright: all 3 category session sizes updated correctly (18/18/20), the new `export-noun.mp3`/`export-verb.mp3` audio both fetch successfully with distinct sizes, screenshot-reviewed the select and drilling screens. Zero console/page errors.

### Homophones Drill — 6th pronunciation template ✅ COMPLETED (Phases H1–H3, H4 folded in)

- Spec lives in `PronunciationFeature.md` §11. Original tier-list assumption ("near-identical mechanic to Phoneme Drill, zero new tooling") turned out to be wrong and was caught during spec'ing, before any content was built: homophones sound identical by definition, so an isolated-word audio clip carries no information about which spelling is correct — the drill needs a full disambiguating **sentence**, not a word, which meant new audio content and a new generation path rather than reusing the word-level TTS pipeline as-is.
- `resources/js/data/pronunciation/homophones.json` — 8 groups (2 three-way sets: their/there/they're, to/too/two; 6 two-way sets: your/you're, write/right, know/no, hear/here, see/sea, meet/meat), 2 example sentences per spelling, 36 sentences total. Each entry has `spelling` (doubles as both display text and `correctKey`), `hint` (short meaning gloss shown as the choice caption), `sentence`, and `audio`.
- **Caught during data validation, not after**: one sentence ("I want to come too.") accidentally contained two spellings from its own group ("to" and "too"), which would have made the correct answer genuinely ambiguous since there's no fill-in-blank marker — the whole sentence's meaning is what's supposed to determine the one correct spelling. Added a tokenize-and-check validation pass specifically for this failure class (not just "does the sentence contain the target word" — also "does it contain more than one of the group's own spellings"), fixed the one flagged sentence, reconfirmed zero remaining ambiguous sentences.
- **New, deliberately isolated audio generation path**: `scripts/generate-pronunciation-audio.mjs` gained a `sentences` mode (`buildSentenceList()`/`generateSentences()`/`sentenceInstructions()`), writing into a new `public/audio/pronunciation/sentences/` folder — kept entirely separate from `words`/`all` rather than folded in, specifically because of the Word Stress `--force` incident above: this way a `--force` run on `sentences` mode can only ever touch that one folder, never `words/` or `phonemes/`. Generated all 36 clips in one run (36 created, 0 failed, 0 silent files); confirmed via `git status` afterward that only the new `sentences/` folder changed — the isolation held this time.
- `resources/js/components/HomophonesDrill.jsx` — same fullscreen select→drilling→results pattern as `WordStressDrill.jsx`. `buildChoicesForGroup()` dedupes a group's distinct spellings (2 or 3) into `DrillLoop` choice cards (`plainBigText` for the spelling, `hint` as the caption); `buildSession()` tags each pooled sentence item with its own group's choice set before shuffling, so scoring stays correct regardless of shuffle order or Mixed-mode pooling across all 8 groups.
- Wired into `PronunciationDrillPage.jsx` (`type === 'homophones'`) and `UploadPage.jsx`'s `PronunciationLauncher` grid, which grew from 4 buttons to 5 — kept the existing `sm:grid-cols-2` layout (uneven last row) rather than reflowing to 3 columns, matching the same precedent already set by Cambridge's 7-button launcher.
- Verified via temp-QA-user + Playwright: ran a 2-way group ("know / no"), a 3-way group ("their / there / they're"), and Mixed — non-hardcoded scores (2/4, 2/6, 7/20) confirm real per-item scoring; zero failed requests to `/audio/pronunciation/sentences/`; confirmed no stray IPA slashes around the spelling choices; screenshot-reviewed select, both 2- and 3-choice drilling layouts, and the results screen. Zero console/page errors.

### DrillLoop: "← Previous" / "Next →" navigation, replacing the old "Choose Another X" exit link ✅ COMPLETED

- User feedback: the mid-drill "← Choose Another Group/Category/Pair" link (only present on 3 of the 4 drills — -ed Endings never had it) should instead let the teacher step back to a *previous exercise* within the current session, not exit to the group-select screen (the ✕ header button already does that).
- `DrillLoop.jsx` restructured to store answers per item index (`{ [itemIndex]: chosenKey }`) rather than local one-shot `status`/`selectedKey`/`score` state — same pattern already established by `QuizActivity.jsx` ("answers stored per question index so navigating back restores answered state; score recalculates dynamically"). `status`/`selectedKey`/`score` are now all derived from `answers` on every render instead of tracked separately, so they can never drift out of sync with what's actually been answered.
- **Added a "Next →" button too, beyond what was literally asked for**: without it, stepping back via "← Previous" would be a dead end — there was no other way to return to the item you'd been on, since forward progress was previously driven only by answering the *current* item. `answeredCount` (`Object.keys(answers).length`) marks the frontier — the next never-yet-reached item — and "Next" is only enabled up to that point, so a teacher can freely review past answers and always get back to exactly where they left off, but can't skip ahead into unanswered territory. Leaving this out would have been a usability trap, not a smaller feature.
- Previously-answered items are read-only when revisited (choice buttons stay disabled, showing the stored correct/wrong flash exactly as it appeared originally) — re-clicking a choice on an unanswered item is still blocked the same way it always was (`status !== null` guard), just now driven by the stored answer instead of local state.
- This is a single shared-component change, so all 4 drills got it for free in one pass — including `-ed Endings`, which never had any mid-drill navigation before. The 3 drills that previously passed `onExit`/`exitLabel` (Minimal Pairs, Word Stress, Homophones) had those props removed from their `<DrillLoop>` calls since the prop no longer exists.
- Verified via temp-QA-user + Playwright: on `-ed Endings` (previously had nothing here), confirmed "Previous" is disabled at item 1, answering two items then going back twice shows read-only stored state at each step (screenshot: item 2 correctly shows the wrong answer red-flashed and the actual correct answer green, exactly as originally answered) and re-disables "Previous" back at item 1; on Homophones, confirmed "Next" correctly returns to the exact frontier item after rewinding (choices interactive again, not stuck read-only) and stays disabled at the frontier itself; confirmed final score after a full session is unaffected by any back-and-forth navigation in between. Zero console/page errors.

### Homophones content batch 2 — 8 more groups, 2 → 5 sentences per spelling ✅ COMPLETED

- User asked to roughly double the group count and get each spelling to "maybe 20" sentences so it wouldn't need topping up for a long time. Flagged the actual scale that implies (16 groups × ~20 sentences ≈ 700 new sentences + matching TTS audio) before starting — agreed instead to double the groups (8 → 16) but scale sentences-per-spelling up more modestly (2 → 5, ~180 new sentences), consistent with this project's established content strategy of growing in stages based on real classroom usage rather than front-loading a huge pool — same approach already used for DET, Cambridge, Minimal Pairs, and -ed Endings' earlier content batches.
- **First parallel-subagent content batch for Pronunciation** (previously used for DET/Cambridge, not yet for this feature): 4 subagents run concurrently — 2 expanding 4 existing groups each (2→5 sentences/spelling, continuing the audio filename numbering from -3 through -5), 2 authoring 4 brand-new groups each from scratch (5 sentences/spelling). Each subagent wrote to its own scratch file rather than editing `homophones.json` directly, specifically to avoid concurrent-edit collisions on a single shared file — merged and validated centrally afterward via a Node script rather than trusting each subagent's self-report.
- Every subagent prompt explicitly restated the ambiguity bug caught in the first batch (a sentence must not contain more than one spelling from its own group) with the actual "I want to come too." example, since it's a non-obvious failure class that isn't derivable just from reading the schema.
- **New groups added**: buy/by/bye (3-way), flour/flower, son/sun, break/brake, peace/piece, wait/weight, whose/who's, new/knew — all genuine homophones, not BP-specific (unlike the Minimal Pairs work, homophone confusion is an English-spelling issue, not an L1-transfer issue, so no particular need to prioritize Portuguese speakers here).
- Central validation after merging (same checks as the first batch, plus new ones for this larger merge): JSON parses, every group has ≥2 spellings, no duplicate audio paths across the whole file, every sentence contains its own target word verbatim, no sentence contains a second spelling from its own group, no duplicate sentences anywhere in the file, and — new check — every sentence for a given spelling within a group reuses that spelling's exact same `hint` text (catches a subagent inventing a slightly different gloss instead of reusing the established one). All checks passed on the first merge; zero fixes needed.
...
### Silent Letters Drill — 7th pronunciation template ✅ COMPLETED

- Spec lives in `PronunciationFeature.md` §12, the last remaining Tier-1 item from the future-topics catalog. Original one-line brief ("tap-the-silent-letter") assumed a free-tap-any-letter interaction, which would have needed a new component since `DrillLoop`'s choice grid caps at 2–4 options (mapped to A–D labels) and most silent-letter words have more letters than that. Resolved by curating exactly 3 candidate letters per word in the data itself (the silent letter + 2 plausible pronounced ones) — same reasoning as every other drill's hand-curated distractors (`correctSound`, `stressIndex`, etc.) — keeping this a genuine Tier-1 "reuse `DrillLoop` unchanged" build.
- `resources/js/data/pronunciation/silentLetters.json` — 6 categories grouped by which letter is silent (Silent B, K, T, H, W, G), 5 words each, 30 words total: comb/thumb/climb/lamb/debt, knife/know/knee/knock/knight, listen/castle/whistle/often/fasten, hour/honest/heir/exhibit/vehicle, write/wrist/wrong/sword/answer, sign/design/gnome/foreign/campaign.
- **`DrillLoop.jsx` gained one more small opt-in prop**: `showWordLabel` (default `false`) — unlike every prior drill, the student needs to *see* the word's spelling to reason about which letter is silent, so it always-renders `item.displayWord` (no toggle, unlike Homophones' `showReveal`) between the play button and the choice grid. Only `SilentLettersDrill` passes it; the other 3 drills (`MinimalPairsDrill`, `EdEndingsDrill`, `WordStressDrill`, `HomophonesDrill`) are unaffected.
- `resources/js/components/SilentLettersDrill.jsx` — same select→drilling→results pattern as `WordStressDrill.jsx` (same background/`.lg-surface-soft` cards, `softCards`/`plainBigText` on `DrillLoop`). `buildSession()` shuffles each item's 3 `candidates` into choice order per item (not just once per session) so the correct letter isn't always in the same visual position; `correctKey` = `silentLetter`, `displayWord` = the word in uppercase.
- Audio reused the existing plain single-word TTS pipeline — `buildWordList()` in `scripts/generate-pronunciation-audio.mjs` extended with one more loop reading `silentLetters.json`; of the 30 words, 3 (thumb, climb, know) already had audio from earlier batches, so `words` mode (without `--force`, per the Word Stress `--force` lesson) generated only the other 27. Checked for the known silent-5,760-byte-file bug — none found.
- Wired identically to the other drills: `PronunciationDrillPage.jsx` gained a `type === 'silent-letters'` branch; `UploadPage.jsx`'s `PronunciationLauncher` grid grew from 5 to 6 buttons.
- **Verification note**: this session's sandbox couldn't bind a listening port for `php artisan serve`, so verification used the Herd desktop app directly (`http://lesson-generator.test`) instead — same temp-QA-user + Playwright pattern otherwise, script run from the project root so Playwright's install resolved correctly. Verified: Mixed mode plays a 20-item session with a non-hardcoded score (5/20 from always clicking the first choice), a single category (Silent B) correctly shows a 5-item session, `displayWord` renders correctly and isn't clipped for longer words (CAMPAIGN, DESIGN), zero failed audio requests, zero console/page errors. Screenshot-reviewed both the select screen and a drilling screen — visual treatment matches Word Stress/Homophones exactly.
...
### Phase 2 — Writing (B2 First) ✅ COMPLETED

- **Explicit constraint from the user**: this app is a Zoom screen-share display tool — the student cannot type into it. So Writing is pure prompt display, simpler than every other phase: no prep/reveal cycle at all, just a prompt shown on screen while the student writes on paper or in their own document, and the teacher gives feedback live. No capture, no auto-checking, ever.
- **No shared engine reused this time** — deliberately two small, independent components rather than forcing a fit, since Part 1 and Part 2's data shapes differ (question + 2 content points vs. a genre-scoped prompt list) and neither needed anything beyond a plain Prev/Next browser. Consistent with the project's stated preference for small duplication over a premature abstraction (same reasoning `VocabPracticeDrill` used for not reusing Pronunciation's `DrillLoop`).
  - `EssayDrill.jsx` (Part 1) — flat pool of 6 items, no select screen (there's only one implicit set — real Part 1 is always a single compulsory question type). Each item shows a framing sentence, the essay question, the 2 given content points plus a fixed "…and your own idea" line (the real exam's third, open point), and a static "Write about 140–190 words" line — word count is the same for both Writing parts at B2 First, so it's hardcoded in the component rather than repeated per item.
  - `GenreDrill.jsx` (Part 2) — genre-select screen (Article / Email / Review / Story, per the real B2 First genre choice) then a Prev/Next browser scoped to that genre, mirroring `InterviewDrill`'s scenario-select-then-browse shape. 3 prompts per genre.
- `resources/js/data/cambridge/b2/writingEssay.json` (6 items) and `writingGenre.json` (4 genres × 3 items = 12 prompts) — 18 new prompts total, spanning varied everyday B2 themes.
